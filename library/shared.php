...
function setReporting() {
	...
	if (DEVELOPMENT_ENVIRONMENT == true) {
		...
		ini_set('display_errors','On');
	}else if (STAGING_ENVIRONMENT == true) {
		error_reporting(E_ALL);
		ini_set('display_errors','On');
	} else {
		error_reporting(E_ALL);
		...
		ini_set('error_log', ROOT.'tmp'.DS.'logs'.DS.'error.log');
	}
}
...
function callHook() {
	$controllerName = strtolower(isset($_GET['controller']) && $_GET['controller']!=""?$_GET['controller']:"index");
    $actionName = strtolower(isset($_GET['action']) && $_GET['action']!=""?$_GET['action']:$controllerName);
	
	$controller = $controllerName.'Controller';
	$action=$actionName."Action";
	
	$controller=class_exists($controller)?$controller:"indexController";
    $dispatch = new $controller($controllerName,$actionName,isset($_GET['ajaxRequest']) && $_GET['ajaxRequest']==1?1:0);
 
    if ((int)method_exists($controller, $action)) {
        call_user_func_array(array($dispatch,$action),$_GET);
    } else {
...
/*function __autoload($className) {
	require_once (ROOT . 'application' . DS .'helpers'.DS. strtolower('functionshelper') . '.php');
        require_once(ROOT . 'library' . DS .'JWT' .DS.'vendor'.DS.'firebase'.DS.'php-jwt'.DS.'src'.DS.'JWT.php');
        require_once(ROOT . 'library' . DS .'JWT' .DS.'vendor'.DS.'firebase'.DS.'php-jwt'.DS.'src'.DS.'BeforeValidException.php');
        require_once(ROOT . 'library' . DS .'JWT' .DS.'vendor'.DS.'firebase'.DS.'php-jwt'.DS.'src'.DS.'ExpiredException.php');
        require_once(ROOT . 'library' . DS .'JWT' .DS.'vendor'.DS.'firebase'.DS.'php-jwt'.DS.'src'.DS.'SignatureInvalidException.php');
        
     //echo"sdd";
    if (file_exists(ROOT . 'library' . DS . strtolower($className) . '.class.php')) {
        require_once(ROOT . 'library' . DS . strtolower($className) . '.class.php');
    } else if (file_exists(ROOT . 'application' . DS . 'controllers' . DS . strtolower($className) . '.php')) {
        require_once(ROOT . 'application' . DS . 'controllers' . DS . strtolower($className) . '.php');
    } else if (file_exists(ROOT . 'application' . DS . 'models' . DS . strtolower($className) . '.php')) {
        require_once(ROOT . 'application' . DS . 'models' . DS . strtolower($className) . '.php');
    } else if (file_exists(ROOT . 'application' . DS . 'helpers' . DS . strtolower($className) . '.php')) {
        require_once(ROOT . 'application' . DS . 'helpers' . DS . strtolower($className) . '.php');
    }
    else if (file_exists(ROOT . 'library' . DS .'mpdf' .DS. strtolower($className) . '.php')) {
        require_once(ROOT .'library' . DS .'mpdf'. DS . strtolower($className) . '.php');
    }
    else if (file_exists(ROOT . 'library' . DS .'tcpdf' .DS. strtolower($className) . '.php')) {
        require_once(ROOT .'library' . DS .'tcpdf'. DS . strtolower($className) . '.php');
    }
    else {
        // Error Generation Code Here 
    }
