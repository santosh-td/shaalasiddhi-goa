...
class aqsformHelper {
    ...
    function renderTabularFields($question_text, $question) {
        ...
        $table_rows = [];
        $group_id = null;
        foreach($question['sub_questions'] as $key=>$sq){
            $group_id = $sq['group_id'] == null ? 0 :  $sq['group_id'];
            if(empty($table_rows[$group_id])){
                $table_rows[$group_id] = [];
            }
            $table_rows[$group_id][$sq['school_profile_id']] = $sq;
        }
        $column_count = count($table_rows[$group_id]);
        ...
        <table class="question-template-table">
            <thead>
                <tr>
                <?php
                    foreach($table_rows[$group_id] as $sq){
            ...
            <tbody>
                ...
                ?>
                    <?php if($plus==1){?>
                    <td><a href="#" class="add-tabular-question-row btn btn-primary">&plus;</a></td>
                <?php }else { ?>
                    <td><a href="#" class="remove-tabular-question-row btn btn-primary">&times;</a></td>
                <?php }?>
